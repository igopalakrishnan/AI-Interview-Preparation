...
const InterviewPrep = () => {
  ...
  const generateConceptExplanation = async (question) => {
    try {
      ...
      if (response.data) {
        let explanationPayload = response.data;

        // If explanation is null but raw exists, try parsing raw JSON
        if (!explanationPayload.explanation && explanationPayload.raw) {
          try {
            const parsed = JSON.parse(explanationPayload.raw);

            if (Array.isArray(parsed.answer)) {
              explanationPayload = {
                ...explanationPayload,
                title: parsed.answer[0].title,
                explanation: parsed.answer[0].explanation,
              };
            } else if (parsed.answer?.explanation) {
              explanationPayload = {
                ...explanationPayload,
                title: parsed.answer.title,
  ...
  const uploadMoreQuestions = async () => {
    try {
      ...
      if (response.data?.session?.questions) {
        setSessionData((prev) => ({
          ...prev,
